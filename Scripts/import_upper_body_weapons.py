...
Imports 7 Weapon Family Upper Body Combat textures, configures pixel filtering (TF_NEAREST, TMGS_NO_MIPMAPS),
...
and their per-action/direction flipbooks in /Game/art/characters/UpperBodyWeapons.
"""
...
def run():
    ...
    unreal.log("=== Project Ascendant: Importing 7 Weapon Family Upper Body Sets into UE5 Paper2D/PaperZD ===")

    asset_tools = unreal.AssetToolsHelpers.get_asset_tools()
    content_dir = unreal.Paths.project_content_dir()
    weapons_disk = os.path.join(content_dir, "art", "characters", "UpperBodyWeapons")
    weapons_ue = "/Game/art/characters/UpperBodyWeapons"
    ...
    for fam_name in family_names:
        unreal.log(f"--- Processing Family: {fam_name} ---")
        fam_disk_dir = os.path.join(weapons_disk, fam_name)
        frames_disk_dir = os.path.join(fam_disk_dir, "frames")
        fam_ue_path = f"{weapons_ue}/{fam_name}"
        textures_ue_path = f"{fam_ue_path}/textures"
        sprites_ue_path = f"{fam_ue_path}/sprites"
        ...
        unreal.log(f"Found {len(frame_files)} frame PNGs for {fam_name}")
        ...
        # 3. Configure pixel filtering & build UPaperSprite
        all_created_sprites = []
        for file_path in frame_files:
            tex_name = os.path.splitext(os.path.basename(file_path))[0]
        ...
        unreal.log(f"Created {len(all_created_sprites)} UPaperSprite assets for {fam_name}")

        # 4. Create Master Flipbook in /Game/art/characters/UpperBodyWeapons/
        master_fb_pkg = f"{weapons_ue}/{fam_name}"
        master_fb = asset_tools.create_asset(fam_name, weapons_ue, unreal.PaperFlipbook, flipbook_factory)
        if master_fb:
            key_frames = []
        ...
        # 5. Create sub-flipbooks per action/direction
        for action in actions:
            for direction in directions:
                prefix = f"{fam_name}_{action}_{direction}"
                matching_sprites = [sp for name, sp in all_created_sprites if prefix in name]
                if matching_sprites:
                    fb_sub_name = f"{fam_name}_{action.capitalize()}_{direction}"
                    sub_fb_pkg = f"{fam_ue_path}/{fb_sub_name}"
                    sub_fb = asset_tools.create_asset(fb_sub_name, fam_ue_path, unreal.PaperFlipbook, flipbook_factory)
                    if sub_fb:
                        kfs = []
                        for sp in matching_sprites:
                            ...
                            kf.set_editor_property('frame_run', 1)
                            kfs.append(kf)
                        fps = 8.0 if action == "guard" else 12.0
    ...
    unreal.log("=== All 7 Weapon Family Upper Body Sets Successfully Imported into Unreal Engine! ===")
